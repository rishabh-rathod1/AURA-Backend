...
const ControllerDisplay = ({ socket }) => {
  ...
  // Add and remove event listeners for gamepad connection/disconnection
  useEffect(() => {
  ...
  const triggerHapticFeedback = (gamepad, intensity = 1.0, duration = 200) => {
  ...
  // Update controller state and handle haptic feedback
      ...
      if (gamepad) {
        ...
        gamepad.buttons.forEach((button, index) => {
          if (button.pressed) {
            ...
            triggerHapticFeedback(gamepad, 1.0, 100); // Full vibration
          }
        });

        // Trigger haptic feedback for joystick movements
        gamepad.axes.forEach((axisValue, index) => {
          const threshold = 0.2; // Minimum movement required
          if (Math.abs(axisValue) > threshold) {
            const intensity = Math.min(Math.abs(axisValue), 1.0); // Scale intensity
            console.log(`Axis ${index} moved: ${axisValue}`);
            triggerHapticFeedback(gamepad, intensity, 100); // Proportional vibration
          }
        });
  ...
  }, [controllerIndex, isConnected, socket]);

  // Animation loop for updates every 0.1 seconds
  useEffect(() => {
    let intervalId;

    if (isConnected) {
      intervalId = setInterval(updateControllerState, 200); // 100ms = 0.1 seconds
    }
  ...
  }, [updateControllerState, isConnected]);

  // Rest of your component code remains the same...
  // Render button state
  const renderButton = (buttonId, buttonValue) => {
    const element = document.getElementById(`controller-b${buttonId}`);
    if (element) {
      const selectedButtonClass = "selected-button";
      if (buttonValue > 0) {
        element.classList.add(selectedButtonClass);
        element.style.filter = `contrast(${buttonValue * 150}%)`;
      } else {
        element.classList.remove(selectedButtonClass);
  ...
  const renderStick = (elementId, leftRightAxis, upDownAxis) => {
    const multiplier = 25; // Scale joystick movement
  ...
  // Update rendering based on controller state
  useEffect(() => {
    controllerState.buttons.forEach((buttonValue, index) => {
      renderButton(index, buttonValue);
    });

    renderStick("controller-b10", controllerState.axes[0], controllerState.axes[1]); // Left stick
    renderStick("controller-b11", controllerState.axes[2], controllerState.axes[3]); // Right stick
  }, [controllerState]);
  ...
  return (
    <div>
     ...
     C268.527,201.874,269.353,202.859,270.437,203.63z"
          ...
          <p className="mt-14 pt-14">No controller connected. Please connect a controller and press a button.</p>
        )}
      </div>
    </div>
  );
